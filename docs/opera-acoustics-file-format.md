...
| `st_condition_declared` | bool | `false` | ST 系 (ST_early / ST_late) の測定条件 (舞台上・音源から 1 m・空席、ISO 3382-1 Annex C) を利用者が自己申告したか。**false (既定) ならキー自体を書かない**。申告が無い限り ST の値は「測定条件不適合」として表示されない (要求 §3.2 の 3 値表示規則) |
...
2. **未知キー保持**: ロードは `value(key).toXxx(現在値)` 方式のため、
   欠落キー = 既定値。新キー追加は旧リーダーを壊さない。
   さらに **他ツールが書いた未知キーは保存時にそのまま書き戻される**
   (ADR-0003 案B、2026-08-15 実装)。ロード時に「ファイル − この版が
   書き戻す内容」の差分を `Project::ofdxExtra()` に保持し、save が
   マージする。既知キーは常に新しい値が勝つ (差分方式なので stale の
   復活は起きない)。未知キーの無いファイルでは出力はバイト単位で不変。
